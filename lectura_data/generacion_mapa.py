...
def fun_generacion_mapa(yacimiento_coords, restricciones_data, rectangles):
    ...
    for i, rect in enumerate(rectangles):
        if rect:  # Ensure the rectangle data is not empty
            ...
            height,width,center_x, center_y  = rect
            ...
            rectangle = patches.Rectangle((bottom_left_x, bottom_left_y), width, height,
                                          linewidth=1, edgecolor='g', facecolor='none', label='Pads')
            plt.gca().add_patch(rectangle)
            plt.plot(center_x, center_y, 'gx', label='Center' if i == 0 else "")
